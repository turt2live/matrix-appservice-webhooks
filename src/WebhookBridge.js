...
class WebhookBridge {
        ...
        return WebhookStore.getAccountData(intent.getClient().credentials.userId).then(async botProfile => {
            ...
            if ((!avatarUrl || avatarUrl !== desiredAvatarUrl) && desiredAvatarUrl) {
                ...
                if (!desiredAvatarUrl.startsWith("mxc://"))
                    ...
                    promisesTasks.push(()=>uploadPromise.then(mxcUrl => {
                    LogService.verbose("WebhookBridge", "Avatar MXC URL = " + mxcUrl);
            ...
            promisesTasks.push(()=>intent.getProfileInfo(intent.getClient().credentials.userId, 'displayname',false)
                .then(profile => {
                    if (profile.displayname != desiredDisplayName) {
                        ...
                        return intent.setDisplayName(desiredDisplayName);
                    }
                }));
            
            
            
            return util.runSerialPromise(promisesTasks);
        });
    }
     ...
     */
    _bridgeKnownRooms() {
        const process = (roomId) => {
            this._processRoom(roomId).catch(e => {
